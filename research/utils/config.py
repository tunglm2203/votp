...
from research.utils.offline_rl_trainer import OfflineRLTrainer
from research.utils.offline_pbrl_trainer import OfflinePbRLTrainer
...
class Config(BareConfig):
    ...
    def get_train_env(self):
        assert self.parsed
        # We need to return an environment with the correct spaces. If we don't have a train env, use the eval env to create one.
        if self["env"] is None:
            ...
            env = research.envs.base.Empty(observation_space=eval_env.observation_space, action_space=eval_env.action_space)
            del eval_env
    ...
    def get_schedules(self):
        ...
        return schedulers_class, schedulers_kwargs
    ...
    def get_model(self, device: Union[str, torch.device] = "auto"):
        ...
        validation_dataset_class = None if self["validation_dataset"] is None else vars(research.datasets)[self["validation_dataset"]]
    ...
    def get_trainer(self):
        ...
        if self["trainer_kwargs"].get("eval_fn", None) is None or self["trainer_kwargs"].get("subproc_eval", False) == True:
            eval_env = None
        else:
            eval_env = self.get_eval_env()
        # Return the trainer
        if self.config["alg"] in ["IQL"]:
            return OfflineRLTrainer(eval_env, **self["trainer_kwargs"])
        elif self.config["alg"] in ["PIQL"]:
            return OfflinePbRLTrainer(eval_env, **self["trainer_kwargs"])
        else:
