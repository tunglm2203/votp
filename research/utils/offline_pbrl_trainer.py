...
class OfflinePbRLTrainer(object):
    def __init__(
        self,
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        if self._train_dataloader is None:
            ...
            self._train_dataloader = torch.utils.data.DataLoader(
                # self.model.dataset.replay_buffer,   # only use replay buffer for policy training
                self.model.dataset,
                shuffle=shuffle,
    ...
    def train_reward_and_relabel_offline_dataset(self, logger, env_domain):
        # Start training reward model
        self.model.train()
        feedback_iter = iter(self.model.dataset.feedback_dataset)   # Prepare 'dataloader' for reward training
        for pretrain_step in tqdm(range(self.model.reward_steps), desc=f"Reward Training - {self.eval_env.unwrapped.spec.name}", leave=False):
            batch = next(feedback_iter, None)   # Mimic ReplayAndFeedbackBuffer to make infinite feedback dataloader
            if batch is None:
                feedback_iter = iter(self.model.dataset.feedback_dataset)
                batch = next(feedback_iter, None)

            batch = self.model.format_batch(batch)
            metrics = self.model.train_reward_step(batch, env_domain=env_domain)

            if 'reward' in self.model.schedulers.keys():
                scheduler = self.model.schedulers['reward']
                scheduler.step()
                metrics.update({'lr_reward': scheduler.get_last_lr()[0]})

            if pretrain_step % 100 == 0:
                log_from_dict(logger, metrics, "reward_training")
                logger.dump(step=pretrain_step, eval=True)

        if 'reward' in self.model.schedulers.keys():
            self.model.schedulers.pop('reward')     # Ignore reward scheduler to avoid conflict with policy scheduler

        # Start relabeling rewards for offline dataset
        infer_bs = 1024
        self.model.eval()
        if env_domain == "metaworld":
            N_DATA_SAMPLES = len(self.model.dataset.replay_buffer._storage['reward'])
            rewards = np.zeros_like(self.model.dataset.replay_buffer._storage['reward'])
        else:
            N_DATA_SAMPLES = len(self.model.dataset.replay_buffer._reward_buffer)
            rewards = np.zeros_like(self.model.dataset.replay_buffer._reward_buffer)

        idxs = np.arange(0, N_DATA_SAMPLES)
        for i in tqdm(range(int(np.ceil(N_DATA_SAMPLES / infer_bs))), desc='Reward Relabeling', leave=False):
            start, end = i * infer_bs, min((i + 1) * infer_bs, N_DATA_SAMPLES - 1)
            # original_idxes = idxs[start:end]
            obs_idxes = idxs[start:end]
            # rewards[i]=r_hat(obs[i-1], action[i]);
            act_idxes = obs_idxes + 1
            if env_domain == "metaworld":
                replay_obs = self.model.dataset.replay_buffer._storage['obs'][obs_idxes]
                replay_act = self.model.dataset.replay_buffer._storage['action'][act_idxes]
            else:
                replay_obs = self.model.dataset.replay_buffer._obs_buffer[obs_idxes]
                replay_act = self.model.dataset.replay_buffer._action_buffer[act_idxes]

            obs = self.model.format_batch(replay_obs)
            action = self.model.format_batch(replay_act)
            with torch.no_grad():
                r_hat = self.model.network.reward(obs, action).mean(axis=0)

            rewards[act_idxes] = r_hat.cpu().numpy()  # index 0 of reward array is dummy (d4rl_dataset.py:74), never sampled (sampling.py:118), so rewards[0] staying 0 is harmless.

        # Replace new rewards generated from reward model
        if env_domain == "metaworld":
            self.model.dataset.replay_buffer._storage._buffers['reward'] = rewards
        else:
            self.model.dataset.replay_buffer._reward_buffer = rewards

        # Log the learned reward scale (root cause of seed-to-seed Q-magnitude spread).
        relabel_metrics = dict(
            relabel_reward_mean=float(rewards.mean()), relabel_reward_std=float(rewards.std()),
            relabel_reward_min=float(rewards.min()), relabel_reward_max=float(rewards.max()),
        )
        log_from_dict(logger, relabel_metrics, "reward_training")
        logger.dump(step=self.model.reward_steps, eval=True)
    ...
    def train(self, path: str):
        ...
        eval_history = []
        self.train_reward_and_relabel_offline_dataset(logger, env_domain)

        self.model.train()
        start_time = time.time()
        current_time = start_time
        pbar = tqdm(total=self.total_steps + 1, desc=f"Policy Training - {self.eval_env.unwrapped.spec.name}")
        offset_step = self.model.reward_steps
        step = 0
        while step <= self.total_steps:
            for batch in self.train_dataloader:
                ...
                if step % self.log_freq == 0:
                    ...
                    logger.dump(step=step + offset_step)
                ...
                if step % self.eval_freq == 0 and not (step == 0 and self.benchmark):
                    ...
                    logger.dump(step=step + offset_step, eval=True)  # Eval Logger dump to CSV: Mark True on eval flag
