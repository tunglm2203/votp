    ...
    **kwargs,
    ...
    for y_key in y_keys:
        ...
        if all(len(xs[0]) == len(x) for x in xs):
            ...
            if "inverse-preference-learning" in paths[0]:
                if not np.all(xs[0:1] == xs):
                    assert (np.unique(xs[:, 0]) == [0, 1]).all(), "X Values must be the same"
            else:
            ...
            max_last_5 = []
            avg_last_5 = []
            if path.split('/')[6] in ["can_mh", "can_ph", "lift_mh", "lift_ph"]:
                n_last = 3
            else:
                n_last = 5
            for i in range(ys.shape[0]):
                max_last_5.append(np.max(ys[i, -n_last:]))
                avg_last_5.append(np.mean(ys[i, -n_last:]))
            max_last_5 = np.array(max_last_5)
            avg_last_5 = np.array(avg_last_5)
            max_last_5_mean = np.mean(max_last_5)
            max_last_5_std = np.std(max_last_5)
            avg_last_5_mean = np.mean(avg_last_5)
            avg_last_5_std = np.std(avg_last_5)
            # print(colored(f"MAX: ", "blue", attrs=["bold"]) + colored(f"{max_mean * 100:.1f} +- {max_std * 100:.1f} ", "green") + f"[{ys.shape[0]} seeds] [step={np.argmax(means)}] [{label}]")
            print(
                colored(f"MAX-LAST-{n_last}: ", "blue", attrs=["bold"]) + colored(f"{max_last_5_mean * 100:.1f} +- {max_last_5_std * 100:.1f} ", "green") +
                f"[{ys.shape[0]} seeds] [{label}], score={np.round(max_last_5 * 100, decimals=2).tolist()}")
            print(
                colored(f"LAST: ", "blue", attrs=["bold"]) + colored(f"{means[-1] * 100:.1f} +- {stds[-1] * 100:.1f} ", "green") + f"[{ys.shape[0]} seeds] [{label}]")
            print(
                colored(f"AVG-LAST-{n_last}: ", "blue", attrs=["bold"]) + colored(f"{avg_last_5_mean * 100:.1f} +- {avg_last_5_std * 100:.1f} ", "green") +
                f"[{ys.shape[0]} seeds] [{label}], score={np.round(avg_last_5 * 100, decimals=2).tolist()}")
            print()
            xs = xs.flatten()
            ys = ys.flatten()
        else:
