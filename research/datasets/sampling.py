...
def sample_qlearning(
    ...
    reward = np.zeros_like(storage["reward"][rew_idxs])
    discount_batch = np.ones_like(storage["discount"][rew_idxs])
    for i in range(nstep):
        reward += discount_batch * storage["reward"][rew_idxs + i]
        discount_batch *= discount * storage["discount"][rew_idxs + i]
