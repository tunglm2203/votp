...
class D4RL_OfflineDataset(ReplayBuffer):
    """
    ...
    def _data_generator(self):
        ...
        if self.normalize_reward:
            ...
            for i in range(len(dataset["observations"])):
                ...
                if done:
                    ...
                    ep_reward, ep_length = 0, 0
            min_reward, max_reward, avg_reward = min(ep_rewards), max(ep_rewards), np.mean(ep_rewards)
            print(f"[research] Normalized D4RL range: min={min_reward:.2f}, max={max_reward:.2f}, mean={avg_reward:.2f}")
        ...
        discount_ = [1.0]
        original_indices = []
        # NOTE: The 'original_indices' are indices of 'obs' at here corresponding to 'observations' in env.get_dataset()
        ...
        for i in range(dataset["rewards"].shape[0]):
            ...
            if done:
                ...
                # This is same with logical above, since the reward for this "next_observations" always be masked out,
                # therefore we just repeat the original indices here, it shouldn't matter for learning Q value.
                # Note that, at the end of episode, the "observations" is actually "next_observations",
                # which is DIFFERENT to "observations" in original dataset
                original_indices.append(i)

                obs_ = np.array(obs_).astype(np.float32)
                action_ = np.array(action_).astype(np.float32)
                ...
                kwargs["original_indices"] = np.array(original_indices)
