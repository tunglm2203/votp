...
class FixedStorage(Storage):
    ...
    def __init__(self, buffers: Dict) -> None:
        ...
        _starts, _ends = [], []
        ep_idx = -1
        for i in range(self._buffers["done"].shape[0]):
            if self._buffers["done"][i] and len(_ends) == ep_idx:
                # Only add new end when start new episode (below condition satisfied)
                _ends.append(i)

            if self._buffers["terminal"][i - 1] and not self._buffers["terminal"][i]:
                ep_idx += 1
                _starts.append(i)

        self._ends = np.array(_ends)
        self._starts = np.array(_starts)
        self._lengths = self._ends - self._starts + 1
        assert len(self._starts) == 2500 and len(self._ends) == 2500
