...
class ReplayBuffer(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        np.random.seed(42)
        self.dummy_action = np.clip(np.random.randn(*self.action_space.sample().shape), self.action_space.low, self.action_space.high)
    ...
    def sample(self, batch_size: Optional[int] = None, stack: int = 1, pad: int = 0) -> Dict:
        ...
        kwargs = utils.get_from_batch(self._kwarg_buffers, obs_idxs)  # Original indices
        if self.next_obs:
